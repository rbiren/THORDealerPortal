...
function ThorLogo({ collapsed = false }: { collapsed?: boolean }) {
  if (collapsed) {
    return (
      <div className="font-heading font-extrabold text-xl">
        <span className="text-burnt-orange">T</span>
      </div>
    )
  }
  return (
...
const navItems = [
  ...
  { href: '/rv-inventory', label: 'RV Inventory', icon: 'M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4' },
  { href: '/vehicle-orders', label: 'Vehicle Orders', icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
  { href: '/products', label: 'Parts & Accessories', icon: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4' },
  ...
  { href: '/orders', label: 'Parts Orders', icon: 'M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z' },
  ...
  { href: '/documents', label: 'Documents', icon: 'M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z' },
  { href: '/warranty', label: 'Warranty', icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },
  { href: '/incentives', label: 'Incentives', icon: 'M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7' },
  ...
  { href: '/forum', label: 'Community', icon: 'M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a1.994 1.994 0 01-1.414-.586m0 0L11 14h4a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2v4l.586-.586z' },
...
const adminItems = [
  ...
  { href: '/admin/orders', label: 'Manage Orders', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01' },
  { href: '/admin/chat', label: 'Live Chat', icon: 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z' },
  { href: '/admin/chat/logs', label: 'Chat Logs', icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
  ...
  { href: '/admin/incentives', label: 'Incentives', icon: 'M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7' },
...
export function SidebarLayout({ user, children }: SidebarLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-light-beige">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 ${isCollapsed ? 'w-sidebar-collapsed' : 'w-sidebar'} bg-charcoal z-sidebar transition-all duration-300 flex flex-col`}
      >
        {/* Logo */}
        <div className="h-header flex items-center justify-between px-4 border-b border-charcoal-800">
          <Link href="/dashboard" className="flex items-center">
            <ThorLogo collapsed={isCollapsed} />
          </Link>
          <button
          ...
          <div className="space-y-1">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`flex items-center px-3 py-2.5 text-sm font-medium text-medium-gray rounded-md hover:text-off-white hover:bg-charcoal-800 transition-colors ${isCollapsed ? 'justify-center' : ''}`}
      ...
      {/* Main Content */}
      <div className={`transition-all duration-300 ${isCollapsed ? 'ml-sidebar-collapsed' : 'ml-sidebar'}`}>
        {/* Header */}

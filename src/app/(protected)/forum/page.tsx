...
export default function ForumPage() {
  ...
  const hasActiveFilters = categoryId || postType !== 'all' || search
  ...
  const loadData = useCallback(async () => {
    ...
    setIsLoading(false)
  }, [search, categoryId, postType, page])
  ...
  }, [search, categoryId, postType, page, router])
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Categories Section */}
      {showCategories && !categoryId && categories.length > 0 && (
        <div className="mb-8">
          ...
          >
            ...
            {hasActiveFilters && (
              <span className="ml-2 px-1.5 py-0.5 bg-primary text-white text-xs rounded">
                {[categoryId, postType !== 'all'].filter(Boolean).length}
              </span>
            )}
          </button>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        <div className="px-6 py-3 border-b border-gray-100 bg-gray-50">
          <h3 className="font-medium text-gray-700">
            {categoryId ? 'Posts' : 'Recent Discussions'}
          </h3>
        </div>
